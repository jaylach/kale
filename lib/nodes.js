...
var ActionNode = function ActionNode(name, args) {
  ...
  if ( args != null ) {
    _.forEach(args, function(arg) {
      if ( arg.nodeType === 'ACCESSOR' ) {
        var props = [ arg.base ].concat(arg.properties);
        tempArgs.push('this._lookupProperty($input, $, ' + JSON.stringify(props) + ')');
      }
      else {
        tempArgs.push(arg.getValueString());
      }
    });
...
ActionNode.prototype.getActionString = function getActionString(varName) {
  return varName + "=this._callAction('" + this.name + "'," + varName + "," + this.args + ");\n" +
         "if ( " + varName + ".$_keep === false ) return null;";
}; //- getActionString()
...
TemplateDefNode.prototype.codeEnd = function codeEnd(args) {
  var code = "return $final;}).bind(this);" +
             ...
             "result = [];" +
             "input.forEach(function(item) { var t = $tplFunction(item, locals); " +
...
// getValue()
ValueNode.prototype.getValue = function getValue() {
  var value = null;
  ...
  switch ( this.type ) {
    ...
    case 'JSON': {
      value = {};
      _.forEach(this.value, function(prop) {
        value[prop.key] = prop.value.getValue();
      });
      break;
    }
  }  
...
ValueNode.prototype.getValueString = function getValueString() {
  ...
  switch ( this.type ) {
    ...
    case 'JSON': {
      value = {};
      _.forEach(this.value, function(prop) {
        value[prop.key] = prop.value.getValueString().replace(/"/g, '');
      });
    ...
    case 'BINDING': {
      ...
      value = 'this._lookupProperty($input, $, ' + JSON.stringify(props) + ')';
      break;
    }
  }
