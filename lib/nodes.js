...
TemplateNode.prototype.compile = function compile(data) {
  ...
  if ( this.imports != null ) {
    ...
    imports = this.imports.map(function(imp) {
      var importName = '$$' + imp.name;
      data.importStack.push(importName);

      return 'var ' + importName + ' = require("' + imp.file + '");';
    });
  }
...
ImportNode.prototype.compile = function compile(data) {
  ...
  return output
    .add('var ' + this.name + ' = require("' + this.file + '");');
}; //- compile()
...
AccessorNode.prototype.compile = function compile(data) {
  ...
  return this._sn(data.fileName, '$engine.lookupProperty($input, $, ' + JSON.stringify(props) + ')');
}; //- compile()
...
ActionNode.prototype.compile = function compile(data) {
  ...
  if ( Array.isArray(data.importStack) && data.importStack.indexOf('$$' + this.action) >= 0 ) {
    return output.add(varName + ' = $$' + this.action + '(' + argsName + ');');
  }
  else {
    return output.add(varName + ' = $engine.callAction("' + this.action + '", ' + varName + ', ' + argsName + ');');
  }
}; //- compile()
