...
rules: [/^(?:\s+)/,/^(?:(\r?\n)+)/,/^(?:--.*(\r|\n))/,/^(?:"[^\"]*"|'[^\']*')/,/^(?:object\b)/,/^(?:property\b)/,/^(?:prop\b)/,/^(?:array\b)/,/^(?:as\b)/,/^(?:if\b)/,/^(?:then\b)/,/^(?:elseif\b)/,/^(?:else\b)/,/^(?:return\b)/,/^(?:end\b)/,/^(?:true\b)/,/^(?:false\b)/,/^(?:null\b)/,/^(?:<=)/,/^(?:>=)/,/^(?:is\b)/,/^(?:==)/,/^(?:isnt\b)/,/^(?:!=)/,/^(?:([@+\-*/%^<>=(){}\[\]:,._]))/,/^(?:and\b)/,/^(?:or\b)/,/^(?:not\b)/,/^(?:&&)/,/^(?:\|\|)/,/^(?:!)/,/^(?:key\b)/,/^(?:(^(\d*\.?\d+)(?:e[+-]?\d+)?))/,/^(?:(^([$A-Za-z_\x7f-\uffff][$\w\x7f-\uffff]*)([^\n\S]*(?!))?))/,/^(?:$)/],
